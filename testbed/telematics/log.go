...
func log(interval, duration uint, nodesFile, dataDir string, num_shards uint, targetfile string, grafana bool, display bool) {
	if display{
		...
		for {
			...
			case now := <-t.C:
				...
				cavg := Snapshot{
					...
					Confirmed_transactions:         ctot.Confirmed_transactions / len(curr) * int(num_shards),
					Deconfirmed_transactions:       ctot.Deconfirmed_transactions / len(curr) * int(num_shards),
				...
				pavg := Snapshot{
					...
					Confirmed_transactions:         ptot.Confirmed_transactions / len(prev) * int(num_shards),
					Deconfirmed_transactions:       ptot.Deconfirmed_transactions / len(prev) * int(num_shards),
				...
				if (counter%300==0) {
					generated_tx_rrb = cavg.Generated_transactions - old_generated_tx
					confirmed_tx_rrb = cavg.Confirmed_transactions - old_confirmed_tx
					old_generated_tx = cavg.Generated_transactions
					old_confirmed_tx = cavg.Confirmed_transactions
					f, _ := os.OpenFile(targetfile, os.O_APPEND|os.O_WRONLY, 0644)
					f.WriteString(fmt.Sprintf("%d, %d\n", generated_tx_rrb/300, confirmed_tx_rrb/300))
				}
				counter = counter + 1
