        ...
        loop {
            ...
            if header_hash < self.header.difficulty {
                ...
                if !skip {
                    ...
                    //For sharding
                    println!("Mined some block, debug id abcd12");
                    let hash = mined_block.hash();
                    self.blockdb.insert(&mined_block).unwrap();
                    let shard_id: u32 = SHARD_ID.load(Ordering::Relaxed);
                    let mut mined_block_content_id: u32 =0;
                    match &mined_block.content {
                        Content::Transaction(_) => {
                            mined_block_content_id =2;
                        }
                        Content::Voter(_) => {
                            mined_block_content_id=1;
                        }
                        Content::Proposer(_) => {
                            mined_block_content_id=0;
                        }
                    }
                    let mut validity_bool: bool = true;
                    let serialized_validity_bool = bincode::serialize(&validity_bool).unwrap();
                    self.blockdb.insert_block_validity(&hash, &serialized_validity_bool); //RR_future: Restrict to transaction blocks only
                    //end for sharding
                    new_validated_block(
                        &mined_block,
                        &self.mempool,
                    ...
                    self.server.broadcast(Message::NewBlockHashes(vec![(mined_block_content_id, shard_id, hash)]));
