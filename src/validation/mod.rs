mod proposer_block;
pub mod transaction; //RR_caution: made transaction public to use the check_signature method (can be bypassed in utxodb/mod.rs)
mod voter_block;
...
pub enum BlockResult {
    ...
    MissingReferences(Vec<(u32,u32,H256)>),
...
pub fn check_data_availability(
    ...
    if !parent_availability {
        missing.push((0,0,parent)); //Setting shard id as 0 since it does not matter
    }

    // match the block type and check content
    match &block.content {
        Content::Transaction(content) => {
            //TODO: Ranvir - check with previous code
        }
...
pub fn check_content_semantic(
    ...
    match &block.content {
        ...
        Content::Transaction(content) => {
            ...
            //RR_caution: Check ordering of shard id check
...
pub fn get_sortition_id(hash: &H256, difficulty: &H256) -> Option<u16> {
    ...
    //println!("Miner debug: big_transaction_range/big_proposer_range= {}", big_transaction_range/big_proposer_range );
