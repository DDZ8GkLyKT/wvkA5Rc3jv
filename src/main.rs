...
fn main() {
    ...
    let matches = clap_app!(Prism =>
     ...
     (@arg shard_id: --sid [HASH] default_value("1") "Assigns node/process to one shard") //RR_caution
     (@arg num_shards: --ns [HASH] default_value("1") "Assigns total number of shards for blockdb cf construction")
     ...
     (@arg init_fund_coins: --("fund-coins") [INT] default_value("5000") "Sets the number of coins of the initial fund for each peer")
     ...
     (@arg mempool_size: --("mempool-size") ... [SIZE] default_value("50000") "Sets the size limit of the memory pool")
     (@subcommand keygen =>
    ...
    let shard_id_str = matches.value_of("shard_id").unwrap();
    prism::config::SHARD_ID.fetch_add(shard_id_str.parse::<u32>().unwrap(),Ordering::Relaxed);
    let num_shards_str = matches.value_of("num_shards").unwrap();
    prism::config::NUM_SHARDS.fetch_add(num_shards_str.parse::<u32>().unwrap(),Ordering::Relaxed);
    //*EXAMPLE += 1;
    // init logger
