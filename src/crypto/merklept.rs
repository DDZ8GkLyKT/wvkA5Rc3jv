...
mod tests {
    ...
    fn new_tree() {
        ...
        let merkle_patricia_tree = MerklePatriciaTree::new(&input_data);
        assert_eq!(merkle_patricia_tree.nodes.len(), 15);
        assert_eq!(
            merkle_patricia_tree.nodes[0],
            (&hex!("9d8f0638fa3d46f618dea970df55b53a02f4aa924e8d598af6b5f296fdaabce5")).into()
        );
        assert_eq!(
            merkle_patricia_tree.nodes[13],
    ...
    fn root() {
        let input_data: Vec<hash::H256> = gen_merkle_tree_data!();
        let merkle_patricia_tree = MerklePatriciaTree::new(&input_data);
        let root = merkle_patricia_tree.root();
        assert_eq!(
            root,
            (&hex!("9d8f0638fa3d46f618dea970df55b53a02f4aa924e8d598af6b5f296fdaabce5")).into()
        );
