...
pub struct BlockChain {
    ...
    unreferred_transactions: Mutex<HashSet<(u32,H256)>>, //RR_caution: Not exactly sure what a Hashset is.
...
impl BlockChain {
    ...
    pub fn insert_block(&self, block: &Block) -> Result<()> {
        ...
        let block_sid = block.header.shard_id;
        ...
        match &block.content {
            ...
            Content::Voter(content) => {
                ...
                drop(voter_best);
                //info!("Adding voter block {} at timestamp {}", block_hash, block.header.timestamp);
            }
            Content::Transaction(content) => {
                ...
                let mut unreferred_transactions = self.unreferred_transactions.lock().unwrap();
                unreferred_transactions.insert((block_sid,block_hash));
                drop(unreferred_transactions);
    ...
    pub fn update_ledger(&self) -> Result<(Vec<(u32,H256)>, Vec<(u32,H256)>)> {
        ...
        if let Some(change_begin) = change_begin {
            ...
            let mut removed_transaction_blocks: Vec<(u32,H256)> = vec![];
            let mut added_transaction_blocks: Vec<(u32,H256)> = vec![];
            for block in &removed {
                let t: Vec<(u32,H256)> = get_value!(transaction_ref_neighbor_cf, block).unwrap();
    ...
    pub fn unreferred_transactions(&self) -> Vec<(u32,H256)> {
        // TODO: does ordering matter?
        let unreferred_transactions = self.unreferred_transactions.lock().unwrap();
        let list: Vec<(u32,H256)> = unreferred_transactions.iter().cloned().collect();
        drop(unreferred_transactions);
        return list;
    }
    ...
    fn insert_block() {
        ...
        let new_transaction_block = Block::new(
            *PROPOSER_GENESIS_HASH,
            ...
            [0; 32],
            H256::default(),
            SHARD_ID.load(Ordering::Relaxed),
        );
