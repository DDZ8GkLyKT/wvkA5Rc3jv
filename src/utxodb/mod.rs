...
impl UtxoDatabase {
    ...
    pub fn add_transaction(
        ...
        if t.shard_id != SHARD_ID.load(Ordering::Relaxed){
            panic!("Dealing with transaction of incorrect shard_id");
        }
        ...
        if !t.input.is_empty() {
            ...
            if t.shard_id == SHARD_ID.load(Ordering::Relaxed){
                PERFORMANCE_COUNTER.record_confirm_shard_transaction(&t);
            }
        }
